...
(function () {
    ...
    let userId = localStorage.getItem('red_chat_user_id');
    if (!userId) {
        userId = 'usr_' + Math.random().toString(36).substring(2, 11) + '_' + Date.now();
        localStorage.setItem('red_chat_user_id', userId);
    }

    // 2. Extract configuration from current script tag
    const scriptEl = document.querySelector('script[src*="chat-widget.js"]');
    const businessSlug = scriptEl ? (scriptEl.getAttribute('data-business') || 'dariv') : 'dariv';
    const customUserId = scriptEl ? (scriptEl.getAttribute('data-user-id') || userId) : userId;
    ...
    // 3. Inject CSS styles for launcher and container
    const styleEl = document.createElement('style');
    styleEl.innerHTML = `
        .red-chat-widget-container {
            position: fixed;
    ...
    // 5. Toggle widget visibility and lazy-load iframe with user_id
    function toggleChat() {
        const isActive = iframeWrapper.classList.contains('active');
        const openIcon = document.getElementById('red-chat-icon-open');
        const closeIcon = document.getElementById('red-chat-icon-close');
        ...
        if (isActive) {
            ...
            if (!isIframeLoaded) {
                // ADDED user_id TO IFRAME URL PARAMETERS HERE:
                iframe.src = `${baseUrl}/chat/widget?business=${encodeURIComponent(businessSlug)}&user_id=${encodeURIComponent(customUserId)}`;
                isIframeLoaded = true;
            }
            iframeWrapper.classList.add('active');
            launcher.classList.add('active');
